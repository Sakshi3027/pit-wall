...
if page == "Live Standings":
    ...
    if season_year == 2026:
        ...
        st.success(
            f"🔴 LIVE 2026 season · {drivers.iloc[0]['full_name']} leads "
            f"{drivers.iloc[1]['full_name']} by {gap} pts after {done} races"
        )
    elif season_year == 2025:
        st.info(
            f"2025 Final — {drivers.iloc[0]['full_name']} champion · "
            f"{int(drivers.iloc[0]['points'])} points"
        )
...
# ── Page: Race Analysis ───────────────────────────────────────────────────────
    ...
    if "laps" in st.session_state:
        ...
        fig2 = px.scatter(
            ...
            color="Compound", color_discrete_map=compound_colors, opacity=0.8,
        )
        fig2.update_layout(
            plot_bgcolor="rgba(0,0,0,0)", paper_bgcolor="rgba(0,0,0,0)",
...
# ── Page: Season Championship ─────────────────────────────────────────────────
    ...
    with st.spinner("Loading current standings..."):
        schedule     = get_season_schedule(season_year)
        constructors = get_constructor_standings(season_year)
    ...
    if season_year == 2026:
        st.success(f"🔴 Simulating the LIVE 2026 season — {done} races done, {total_rounds - done} remaining")
    ...
    noise = st.slider(
        "Upset factor (higher = more chaotic season)", 0.05, 0.40, 0.20
    )
    ...
    if st.button("Run Championship Simulation", type="primary"):
        ...
        if round_standings.empty:
            st.error("No standings data found for that round. Try a lower round number.")
            st.stop()

        strengths = build_driver_strengths(round_standings)

        with st.spinner(f"Running {n_sims:,} season simulations..."):
            from app.models.season_simulator import simulate_season as _sim

            def _sim_with_noise(current_standings, remaining_races,
                                driver_strengths, n_simulations):
                import numpy as np
                POINTS = {1:25,2:18,3:15,4:12,5:10,6:8,7:6,8:4,9:2,10:1}
                drivers_  = current_standings["driver"].tolist()
                base_pts  = dict(zip(current_standings["driver"],
                ...
                for _ in range(n_simulations):
                    ...
                    for __ in range(remaining_races):
                        scores = {
                            d: max(0, s + np.random.normal(0, noise))
                            for d, s in driver_strengths.items()
                        }
                ...
                return pd.DataFrame([{
                    "driver": d,
                    "wdc_probability": round(win_counts[d]/n_simulations*100, 1),
                    "avg_final_points": round(final_pts_sum[d]/n_simulations, 1),
                    "current_points": base_pts.get(d, 0),
                } for d in drivers_]).sort_values(
                    "wdc_probability", ascending=False
                ).reset_index(drop=True)

            results = _sim_with_noise(
    ...
    if "sim_results" in st.session_state:
        ...
        with col_l:
            ...
            fig2.add_trace(go.Bar(
                name="Current", x=results["driver"],
                y=results["current_points"], marker_color="#555",
            ))
            fig2.add_trace(go.Bar(
                name="Projected Final", x=results["driver"],
        ...
        with col_r:
            ...
            fig3 = px.bar(
                ...
                color="gap", color_continuous_scale=["#e10600","#333"],
                text="gap",
            )
            fig3.update_layout(
                plot_bgcolor="rgba(0,0,0,0)", paper_bgcolor="rgba(0,0,0,0)",
